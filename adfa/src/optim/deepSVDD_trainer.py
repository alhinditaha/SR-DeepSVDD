...
class DeepSVDDTrainer(object):
    def __init__(self, objective='soft-boundary', R=0.0, c=None, nu=0.1,
        ...
        self.n_jobs_dataloader=n_jobs_dataloader; self.hinge_power=hinge_power
        ...
        for _ in range(1, self.n_epochs+1):
            ...
            for batch in loader:
                ...
                if self.objective=='one-class':
                    loss = d2[y==0].mean() if (y==0).any() else torch.tensor(0.0, device=self.device)
                else:
                    viol=torch.clamp(d2[y==0]-self.R**2, min=0.0)
                    loss_norm = viol.mean() if p==1 else (viol**2).mean()
                    loss = self.R**2 + (1.0/max(self.nu,1e-8))*loss_norm
                opt.zero_grad(set_to_none=True); loss.backward(); opt.step()
            sch.step()
            if self.objective=='soft-boundary': self.R=self.update_radius(net, dataset, q=1.0-self.nu)
    ...
    def test(self, dataset, net):
        ...
        for batch in loader:
            x=batch['x'].to(self.device); y=batch['y'].to(self.device).long()
            z=net(x); d2=torch.sum((z-self.c)**2, dim=1); s=d2 if self.objective=='one-class' else (d2 - (self.R**2))
